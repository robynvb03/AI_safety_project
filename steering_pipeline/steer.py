...
STEERING_LAYER = 6 # 12 layers in gpt2-small
REPETITIONS = 3
...
class prompt_generator:
    ...
    def __call__(self, prompt, steering_vector, m: int, out_file: str = OUTPUT_FILE):
        ...
        # check steering vector shape 
        d_model = self.model.cfg.d_model
        if v.ndim != 1 or v.shape[0] != d_model:
            raise ValueError(f"Invalid Shape: steering_vector must have shape [{d_model}], got {tuple(v.shape)}")
        ...
        outputs = []
        for _ in range(m):
            # hooks context applies during generation
